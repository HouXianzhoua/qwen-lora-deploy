...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/infer.log"), logging.StreamHandler()],
)
...
def generate_response(
    insruction, max_new_tokens=128, temperature=0.7, top_p=0.9, do_sample=False
):
    # 👇 关键修改：使用和训练时一致的格式
...
def safe_generate_response(instruction, **kwargs):
    try:
        ...
        logging.error(
            f"Error generating response for '{instruction}': {e}", exc_info=True
        )
